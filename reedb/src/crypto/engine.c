...
ree_err_t rcry_keygen(unsigned char *key, cryflgs_t flags)
{
  ...
  size_t ksize = gcry_cipher_get_algo_keylen(RCRY_CORE_CIPHER);
  unsigned char *tmp = (unsigned char*) gcry_malloc_secure(ksize * sizeof(unsigned char));

  /* Then fill it with secure random garbage */
  gcry_randomize(tmp, ksize, GCRY_STRONG_RANDOM);

  /* Loop through the binary string and make it human readable */
  int i; 
  for (i = 0; i < ksize; i++, key += 2)
    snprintf (key, 3, "%02x", tmp[i]);


  if(flags == AUTO_USE)
  {
    gcry_error_t key_error = gcry_cipher_setkey(rcry_handle, key, ksize);
    if (key_error) {
